...
const tocSensibilityController = {
  instructions: async(req,res) =>{
    try{
      ...
      return res.render('sensibilityInstructions',{title:'Ejercicios de simulación',data})
  ...
  toc: async(req,res) =>{
    try{
      //specific info
      const idRoute = 2
      const ejsTable = 'tocSensibility'
      const title = 'Sensibilidad TOC'
  ...
  tocValidation: async(req,res) =>{
    try{
      //specific info
      const idRoute = 2
      const ejsTable = 'tocSensibility'
      const title = 'Sensibilidad TOC'
      let dataToCompare = validations.cotSensibility
      const errorName = 'TOC (%) = '
  ...
  viewChart: async(req,res) =>{
    try{
      ...
      const well = await wellsQueries.wellName(idWell)
      const processData = processesData.filter(process => process.name == 'toc-sensibility')[0]
      ...
      var idBackRoute = idRoute == 4 ? idRoute - 2 : idRoute - 1
      const continueRoute = '/' + processData.name + '/' + idWell + processData.routes.filter(route => route.id == idContinueRoute)[0].route
...
module.exports = tocSensibilityController
