...
async function getData(idRoute,idWell){
  const processName = 'hi-sensibility'
...
const hiSensibilityController = {
  hi: async(req,res) =>{
    try{
      //specific info
      const idRoute = 1
      const ejsTable = 'hiSensibility'
      const title = 'Sensibilidad HI'
  ...
  hiValidation: async(req,res) =>{
    try{
      ...
      const ejsTable = 'hiSensibility'
      const title = 'Sensibilidad HI'
      let dataToCompare = validations.hiSensibility
      const errorName = 'HI = '
  ...
  viewChart: async(req,res) =>{
    try{
      ...
      const well = await wellsQueries.wellName(idWell)
      const processData = processesData.filter(process => process.name == 'hi-sensibility')[0]
...
module.exports = hiSensibilityController
