...
async function getData(idRoute,idWell){
  const processName = 'kinetic-sensibility'
...
const kineticSensibilityController = {
  kinetic: async(req,res) =>{
    try{
      //specific info
      const idRoute = 1
      const ejsTable = 'kineticSensibility'
  ...
  kineticValidation: async(req,res) =>{
    try{
        //specific info
        const idRoute = 1
        const ejsTable = 'kineticSensibility'
        const title = 'Sensibilidad Cinética'
        let dataToCompare = validations.kineticSensibility1
        const errorName = 'Cinética = '
  ...
  viewChart: async(req,res) =>{
    try{
      ...
      const well = await wellsQueries.wellName(idWell)
      const processData = processesData.filter(process => process.name == 'kinetic-sensibility')[0]
...
module.exports = kineticSensibilityController
