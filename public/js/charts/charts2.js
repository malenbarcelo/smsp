import { Runtime_2, Library_2, Inspector_2 } from "./runtime2.js"
...
import { definePatterns_2, defineClips_2 } from './patternsAndClips2.js';
...
window.addEventListener('load',async()=>{
  ...
  function _4(d3_2, DOM) {

    let svg2 = d3_2.select(DOM.svg(width, height))
    ...
    {
      defs = definePatterns_2(svg2);
    }

    let x = d3_2.scaleLinear()
      .domain(ejeXConfig.dominio)
      .range([ml, width - mr]);

    let y = d3_2.scaleLinear()
    ...
    let ax = svg2.append('g')
    ax.call(draw, datosCapas, x, y);

    //hide d3_2 bar with info
    ...
    function zoomed() {
      let transform = d3_2.event.transform;
    ...
    function draw(g, data, x, y) {
      if (data[0] && data[data.length - 1]) {
        ...
        defineClips_2(svg2,ml,mr,mt,mb,width,height);
        ...
        let xAxis = g => g.append('g')
          ...
          //.call(d3_2.axisBottom(x).tickValues(XticksMinor.concat(XticksMajor)))
          .call(d3_2.axisBottom(x).tickFormat(d => d))//d => XticksMajor.includes(d) ? d : ""))
          .call(g => {
        ...
        let yAxis = g => g.append('g')
          ...
          //.call(d3_2.axisLeft(y).tickValues(YticksMinor.concat(YticksMajor)))
          //.call(d3_2.axisLeft(y).tickFormat(null))
          .call(d3_2.axisLeft(y).tickFormat(d => d))//d => XticksMajor.includes(d) ? d : ""))
          .call(g => {
            g.selectAll(".tick text")
                .style("font-size", `${proportion * 10}px`)
                .style("font-family", "Arial");
        ...
        g.append("line")      // agregar un elemento line al SVG2
    ...
    function drawAreaBetweenLines(g, data1, data2, gradientId, className, area) {
      ...
      g.selectAll(`path.${className}`)
        ...
        //.attr('fill', d => 'url(#' + texture +')')  // Asumiendo que ya has definido la textura con un ID en el SVG2
    ...
    function drawPlusSigns(g,puntos,x,y,proportion) {
      ...
      g.selectAll('line.vertical-plus')
        ...
        g.selectAll('line.horizontal-plus, line.vertical-plus')
          ...
          .on('mouseout', () => d3_2.select("#tooltip").style("visibility", "hidden"));
    }
    ...
    function onMouseOverLine(event, d ) {
      ...
      let [mx, my] = d3_2.mouse(this);  // Usa d3_2.mouse en lugar de d3_2.pointer
    ...
    function onMouseOverPlus(event, d) {
      let [mx, my] = d3_2.mouse(this);  // Usa d3_2.pointer en lugar de d3_2.mouse si estás usando D3_2 v6 o superior
    ...
    function onMouseOut(event, d) {
      d3_2.select("#tooltip").style("visibility", "hidden");  // Oculta el tooltip
    }

    function onMouseOverCircle(event, d) {
      d3_2.select(this)  // Selecciona el círculo actual
        //.attr('fill', 'red')  // Cambia el color a rojo
        .attr('r', '10');



      let [mx, my] = d3_2.mouse(this);  // Obtén las coordenadas del mouse
    ...
    if (graphicAttributes.colorMap)
    {
      drawColorScale(svg2,ml, mr, mb, height, width, colorPalette,alphaName,proportion);
    }
      return svg2.node()
  }
  ...
  function createGradientForLayer(data, gradientId) {
    ...
    // Crea el elemento SVG2 del gradiente
    const gradient = `
  ...
  function define(runtime, observer) {
    ...
    main.variable(observer("d3_2")).define("d3_2", ["require"], _d3_2);
    main.variable(observer()).define(["d3_2", "DOM"], _4);
    return main;
  }

  const main = runtime2.module(define, Inspector_2.into(myChart2))
})